...
EventPtr checkCircleEvent(bl::BLNodePtr n1, bl::BLNodePtr n2, bl::BLNodePtr n3,
    ...
    if (!Circle::findCircleCenter(p1, p2, p3, center))
        return nullptr;
...
void VoronoiDiagram::build_voronoi(const std::vector<Point2D> &points,
    ...
    while (!pq.empty()) {
        ...
        if (e->type == Event::SITE) { // handle site event
            ...
            if (root == nullptr) { // init empty beachline tree
                ...
                int isp_num = Parabola::intersectionPointsNum(points[arc->get_id()], e->point, sweepline);
