...
void ShapesScene::settingsChanged() {
    // If each setting type has been changed, regenerate vertices/normals and redraw shapes
    if(shapeType != settings.shapeType){
        generateShape();
    }
    else if (param_1 != settings.shapeParameter1){
        param_1 = settings.shapeParameter1;
        generateShape();
    }
    else if (param_2 != settings.shapeParameter2 && shapeType != SHAPE_CUBE){
        param_2 = settings.shapeParameter2;
        generateShape();
    }
}

void ShapesScene::generateShape(){
    switch(settings.shapeType){
        case SHAPE_CUBE:
        m_shape = std::make_unique<Cube>(glm::max(settings.shapeParameter1, 1), settings.shapeParameter2);
            break;
        case SHAPE_CONE:
        m_shape = std::make_unique<Cone>(glm::max(settings.shapeParameter1, 1), glm::max(settings.shapeParameter2, 3));
            break;
        case SHAPE_CYLINDER:
        m_shape = std::make_unique<Cylinder>(glm::max(settings.shapeParameter1, 1), glm::max(settings.shapeParameter2, 3));
            break;
        case SHAPE_SPHERE:
        m_shape = std::make_unique<Sphere>(glm::max(settings.shapeParameter1, 2), glm::max(settings.shapeParameter2, 3));
            break;
        default:
